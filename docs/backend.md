...
- `JsonWebToken`（`encode` / `decode`）でラップ。`exp` に 24h。
- payload：一般 = `{ user_id, role: "user" }`、管理者 = `{ admin_id, role: "admin" }`。
- フロントは Cookie 保存（user_token / admin_token）。`sameSite: strict`、本番 `secure: true`。
