...
- **完了**：#1〜#7（基盤 / データモデル / GraphQL・フロント基盤 / 一般・管理者認証 / 公式単語帳の閲覧）
  - #2 の補足：`admin_users` テーブルは作らず **`users.role` で管理者判定**にしている（別テーブル化は要決定）。sessions テーブル（DB セッション）を追加。
- **進行中**：#8 自作単語帳・単語の CRUD（バックエンドはマージ済み、フロントエンドを実装中）
- **一部先行**：#14 のうち公式単語帳 CRUD の Mutation（`createAdminWordbook` 等）はバックエンドのみ実装済み（管理画面は未実装）
- **未着手**：#9〜#13、#15。復習タグ・章の進捗はフロントのクライアント一時状態で UI のみ先行再現中（[frontend.md](./frontend.md) §3）
