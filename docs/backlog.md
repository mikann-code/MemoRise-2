...
- [ ] 結果画面から誤答単語を復習タグへ一括登録（confirm つき。自動登録はしない）
- [ ] 復習単語一覧 `/wordbooks/review` → 復習専用テスト `/wordbooks/review/test`
- [ ] ホーム等に復習件数バッジ
...
- [ ] マイページ（streak・登録単語数）
- [ ] プロフィール編集（キャッシュ即時反映）
- [ ] 進捗（章の解放：完了で次パート解放、lazy initialization）※復習タグは接続済み。ここでは進捗の永続化のみ残る
