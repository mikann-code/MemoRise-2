...
| レイヤー | v1 | v2 |
| --- | --- | --- |
| データ取得ロジック | `lib/*.ts`（fetch + エラー処理） | GraphQL ドキュメント（`*.graphql`）+ Codegen 生成フック |
| キャッシュ / 状態 | `hooks/use*.tsx`（React Query） | Apollo Client（`useQuery` / `useMutation`） |
| UI 描画 | `components/` | `components/`（MUI ベース） |
| ルーティング / 画面 | `app/` | `app/` |

- v1 では「API が変わったら lib だけ」「キャッシュ戦略が変わったら hooks だけ」という設計だった。v2 では Codegen により**スキーマ変更が型に自動反映**され、この分離がさらに強化される。
...
- v2：**Apollo Client のインスタンス（または link）を一般用・管理者用で分離**し、それぞれの Cookie トークンを `Authorization` ヘッダに付与。キャッシュ空間も分離して汚染を防ぐ（v1 の `["me"]` / `["adminMe"]` キー分離に相当）。
- Cookie：`sameSite: strict`、一般は `secure` を環境で切替、管理者は `secure: true` 固定。
- API ベース URL は `NEXT_PUBLIC_API_URL`（GraphQL エンドポイント）で環境切替。
...
- GraphQL Codegen を CI に組み込み、スキーマとフロント型の乖離を防ぐ。
