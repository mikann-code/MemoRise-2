...
| | |
| :---: | --- |
| <img src="./docs/images/public-01.jpg" width="180" alt="公式単語集の教材トップ：進捗バーと、解放済み・ロック中の章"> | **進捗はサーバー側で保持** — 解放状態はクライアントの一時状態ではなく `wordbookProgresses` に保存。先頭章はサーバーが遅延作成し、章の完了と次章の解放は同一トランザクションで確定させる（`completeWordbookProgress`）。<br><br>**「今すぐはじめる」で迷わせない** — 未完了かつ解放済みの先頭章へ直接飛ばすので、どこから再開するか考えずに済む。<br><br>**解放済みの章はいつでも読み返せる** — テストを受け直さなくても単語リストだけを確認できる。ロック中の章は一覧アイコンごとグレーの非リンクにして、押せないものを押させない。 |
