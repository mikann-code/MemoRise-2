...
- **冗長なものは追加しない**。既存の型・フィールド・関数で取れる値に対して、専用の
  リゾルバ・クエリ・API 面を新設しない（例：`me { wordsCount }` で取れるなら `totalWords` は
  作らない）。同じ値に入口を 2 つ作らず既存に寄せる。集計コスト回避など明確な理由がある時だけ
  専用実装を立てる。
